...
import type { CharacteristicDescription, Profile, RacialMaximums } from '../types';

export const CHARACTERISTIC_DESCRIPTIONS: Record<string, CharacteristicDescription> = {
...
export const RACIAL_MAXIMUMS: RacialMaximums = {
...
export const BS_TO_HIT: Record<number, number> = {
  1: 6, 2: 5, 3: 4, 4: 3, 5: 2, 6: 1, 7: 0, 8: -1, 9: -2, 10: -3
};

// To Hit chart for close combat (attacker WS vs defender WS)
export function getCloseCombatToHit(attackerWS: number, defenderWS: number): number {
...
export function getWoundRoll(strength: number, toughness: number): number | null {
...
export function getArmorSaveModifier(strength: number): number {
